...
(function($) {
  ...
  mergeSettings = function(options) {
    ...
    // to merge the default and options arrays instead of overriding the
    // defaults.
    if (options) {
      if (options.inputs) {
  ...
  processAll = function(skipPushState) {
    ...
    // TODO: Wrap this in a try/rescue block to hide the processing indicator
    // and indicate something went wrong if error
    ...
    if (this.settings.dataset.ajax) {
      ...
      // dynatable
      // that may have been in URL when page loaded, so that it doesn't
      // conflict with
  ...
  function DomColumns(obj, settings) {
    ...
    this.attachGeneratedAttributes = function($cell) {
      // Use increment to create unique column name that is the same each time
      // the page is reloaded,
      // in order to avoid errors with mismatched attribute names when loading
      // cached `dataset.records` array
      ...
      var sortFunction = function(a, b) {
        ...
        if ($.isEmptyObject(sorts)) {
          ...
          for (var i = 0, len = sortsKeys.length; i < len; i++) {
            ...
            // Don't need to sort any further unless this sort is a tie between
            // a and b,
      ...
      tableRecords.each(function(index){
        ...
        $(this).find('th,td').each(function(index) {
          if (columns[index] === undefined) {
            ...
            // append
            // a new header cell with a randomly generated name (so we can
            // store and
          ...
          // If value from table is HTML, let's get and cache the text
          // equivalent for
          // the default string sorting, since it rarely makes sense for sort
          // headers
        ...
        // Allow configuration function which alters record based on attributes
        // of
  ...
  function State(obj, settings) {
    ...
    this.push = function(data) {
      ...
      // Since we don't know what the actual limit will be in any given
      // situation, we'll just try caching and rescue
      // any exceptions by retrying pushState without caching the records.
      //
      // I have absolutely no idea why perPageOptions suddenly becomes an
      // array-like object instead of an array,
      // but just recently, this started throwing an error if I don't convert
      // it:
  ...
  utility = dt.utility = {
    ...
    refreshQueryString: function(urlString, data, settings) {
      ...
      for (attr in settings.params) {
        if (settings.params.hasOwnProperty(attr)) {
          ...
          // Skip over parameters matching attributes for disabled features
          // (i.e. leave them untouched),
          // because if the feature is turned off, then parameter name is a
          // coincidence and it's unrelated to dynatable.
          if (
          ...
          // individually here instead of
          // handling the entire queries object below, since we need to make
          // sure that this is a query controlled by dynatable.
          ...
          // If we haven't returned true by now, then we actually want to
    ...
    allMatch: function(item, arrayOrObject, test) {
      ...
      $.each(arrayOrObject, function(key, value) {
        ...
        // If a single item tests false, go ahead and break the array by
        // returning false
        // and return false as result,
        // otherwise, continue with next iteration in loop
        // (if we make it through all iterations without overriding match with
        // false,
    ...
    anyMatch: function(item, arrayOrObject, test) {
      ...
      $.each(arrayOrObject, function(key, value) {
        var result = isArray ? test(item, value) : test(item, key, value);
        if (result) {
          // As soon as a match is found, set match to true, and return false
          // to stop the `$.each` loop
          match = true;
          return false;
        }
      });
